...
export async function getProductListing(slug: string) {
  return safeQuery(
    () =>
...
export type ProductListing = NonNullable<Awaited<ReturnType<typeof getProductListing>>>;

/** Tienda premium desde la que se está viendo el producto (activa el modo "tienda": sin otros vendedores ni menú general). */
...
export async function ProductDetail({ listing, store }: { listing: ProductListing; store?: StoreContext }) {
  const related = await safeQuery(async () => {
    const baseWhere = {
      ...
      id: { not: listing.id },
      stock: { gt: 0 },
      ...(store ? { sellerId: listing.sellerId } : {}),
    };
  ...
  const canonicalPath = store ? `/tienda/${store.slug}/producto/${listing.slug}` : `/producto/${listing.slug}`;
  const productLd = {
    "@context": "https://schema.org",
    "@type": "Product",
    name: listing.title,
    ...(listing.imageUrl ? { image: [listing.imageUrl] } : {}),
    description: listing.description ?? `${listing.title} — ${gameName(listing.game)}`,
    category: TYPE_LABEL[listing.type] ?? listing.type,
    ...(listing.cardNumber ? { sku: `${listing.game}-${listing.cardNumber}` } : {}),
    brand: { "@type": "Brand", name: gameName(listing.game) },
    offers: {
      "@type": "Offer",
      ...
      seller: { "@type": "Organization", name: store?.name ?? listing.seller.name },
    },
  };
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      <nav className="mb-6 flex flex-wrap items-center gap-1.5 text-[12px] text-ink-400">
        {store ? (
          <>
            <Link href={`/tienda/${store.slug}`} className="hover:text-carbon">
              {store.name}
            </Link>
            <span>/</span>
            <Link href={`/tienda/${store.slug}?game=${listing.game}#catalogo`} className="hover:text-carbon">
      ...
      <div className="grid gap-8 lg:grid-cols-[380px_1fr]">
        ...
        <div>
          ...
          {/* VENDEDOR */}
          <Link
            href={store ? `/tienda/${store.slug}` : `/vendedor/${listing.seller.slug}`}
          ...
          >
            <span className="relative flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-full bg-carbon font-display text-lg font-bold text-paper">
              {(store ? store.logoUrl ?? listing.seller.avatarUrl : listing.seller.avatarUrl) ? (
                <Image
                  src={(store ? store.logoUrl ?? listing.seller.avatarUrl : listing.seller.avatarUrl) as string}
                ...
                (store?.name ?? listing.seller.name).charAt(0).toUpperCase()
              )}
            </span>
            <span className="min-w-0 flex-1">
              <span className="block text-sm font-semibold text-ink-200">
                {store ? store.name : listing.seller.name}
              </span>
              <span className="block text-[11px] text-ink-400">
                {store ? "Tienda oficial" : "Vendedor en Win Condition"}
      ...
      {related.length > 0 && (
        <section className="mt-16">
          <h2 className="mb-4 font-display text-2xl font-bold text-carbon">
            {store ? `Más de ${store.name}` : `También de ${gameName(listing.game)}`}
          </h2>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            {related.map((l) => (
              <ListingCard key={l.id} listing={l} href={store ? `/tienda/${store.slug}/producto/${l.slug}` : undefined} />
            ))}
          </div>
        </section>
      )}
